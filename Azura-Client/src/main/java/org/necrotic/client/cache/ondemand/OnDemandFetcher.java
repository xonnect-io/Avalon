...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
	...
	private void checkReceived() {
		...
		while (onDemandData != null) {
			...
			synchronized (loadRequestList) {
				if (abyte0 == null) {
					next.pushHead(onDemandData);
				} else {
					onDemandData.setBuffer(abyte0);
					synchronized (incompleteList) {
						incompleteList.pushHead(onDemandData);
					}
				}
				onDemandData = (OnDemandRequest) loadRequestList.popHead();
			}
		...
		while (uncompletedCount == 0 && completedCount < 10) {
			...
			while (onDemandData != null) {
				if (fileStatus[onDemandData.getDataType()][onDemandData.getId()] != 0) {
					...
					requested.pushHead(onDemandData);
	...
	private void handleFailed() {
		...
		while (uncompletedCount < 10) {
			...
			try {
				...
				requested.pushHead(onDemandData_1);
				uncompletedCount++;
	...
	public void loadExtra(int file, int cache) {
		...
		synchronized (extraFilesList) {
			extraFilesList.pushHead(onDemandData);
		}
	}
				...
				if (current != null) {
					...
					if (fileLength == 0) {
						...
						if (current.incomplete) {
							synchronized (incompleteList) {
								incompleteList.pushHead(current);
							}
						} else {
							current.unlink();
						}
	...
	private void dumpMapIndex(int j1) throws IOException {
		try {
			...
			for (int id = 0; id < j1; id++) {
				try {
					//
					writer.append("mapIndices1[").append(String.valueOf(id)).append("] = ").append(String.valueOf(mapIndices1[id]));
					writer.newLine();
