	...
	public void displayItemSearch() {
		...
		try {
			if (searching && Client.getClient().inputDialogState == 3) {
				...
				Client.spritesMap.get(1149).drawSprite(0 + xPosOffset, 0 + yPosOffset);
				...
				for (int j = 0; j < totalItemResults; j++) {
					...
					if (yPos > 0 && yPos < 210) {
						...
						Client.getClient().chatTextDrawingArea.method591(Client.capitalizeFirstChar(n), 78, 0xA05A00, yPos + yPosOffset + (totalItemResults < 8 ? 6 : 0));
						if (x > 74 && x < 495 && y > ((GameFrame.getScreenMode() == ScreenMode.FIXED) ? 338 : Client.clientHeight - 165) + yPos - 13 + (totalItemResults < 8 ? 6 : 0) && y < ((GameFrame.getScreenMode() == ScreenMode.FIXED) ? 338 : Client.clientHeight - 165) + yPos + 2 + (totalItemResults < 8 ? 6 : 0)) {
							DrawingArea.fillRect(0x807660, yPos - 12 + yPosOffset + (totalItemResults < 8 ? 6 : 0), 424, 15, 60, 75);
				...
				if (Client.getClient().amountOrNameInput.length() == 0) {
					Client.getClient().chatTextDrawingArea.drawCenteredText(0xA05A00, 259, "Grand Exchange Item Search", 30 + yPosOffset, false);
				...
				DrawingArea.fillRect(0x807660, 121 + yPosOffset, 506, 15, 120, 7);// box
				// chatTextDrawingArea.drawText(0, "<img=8>", 133, 12);
				Client.getClient().chatTextDrawingArea.method591(Client.getClient().amountOrNameInput + "*", 28 + xPosOffset, 0xffffff, 133 + yPosOffset);
				// chatTextDrawingArea.drawText(0xffffff, amountOrNameInput + "*",
				// 133, 122);
